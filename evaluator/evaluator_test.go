...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
        ...
        {
            `{"name": "Monkey"}[fn(x) { x }];`,
            "Index must be of type integer, string or boolean, got: FUNCTION",
        },
	}
	for _, tt := range tests {
		evaluated := testEval(tt.input)
...
func TestBuiltinFunctions(t *testing.T) {
    tests := []struct {
        ...
        {`len("four")`, 4},
        {`len("hello world")`, 11},
        {`len(1)`, "argument of type INTEGER not supported for len()"},
        {`len("one", "two")`, "len() requires exactly one argument, got 2"},
        {"len([1,2,3])", 3},
        {"first([1,2,3])", 1},
        {"last([1,2,3])", 3},
    }
...
func TestHashLiterals(t *testing.T) {
    ...
    for expectedKey, expectedValue := range expected {
        obj, ok := result.Get(expectedKey)
        if !ok {
            t.Errorf("no pair for given key in Pairs")
        }
        testIntegerObject(t, obj, expectedValue)
    }
}
