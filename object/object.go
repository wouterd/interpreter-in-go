...
func NewHash() Hash {
    return Hash{data: make(map[HashKey]HashPair)}
}

func (h *Hash) Get(key Hashable) (Object, bool) {
    pair, ok := h.data[key.HashKey()]
    if !ok {
        return nil, false
    }
...
func (h *Hash) Set(key Hashable, val Object) {
    h.data[key.HashKey()] = HashPair{Key: key, Value: val}
}
